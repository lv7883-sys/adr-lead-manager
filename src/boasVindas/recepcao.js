...
SELECT bt.id, bt.account_id, bt.etapa_id, bt.repeticao, bt.status, bt.bloqueio, bt.motivo, bt.due_at, bt.versao,
       bt.phone, bt.destinatario_nome, bt.cliente_nome, bt.texto_final,
...
async function enviar(tenantId, toqueId, { texto, comAnexo = true, sender = 'RECEPCAO' } = {}, deps = {}) {
  ...
  const reservado = await withTenant(tenantId, async (c) => (await c.query(
    ...
    [tenantId, toqueId, ESTADOS_ABERTOS])).rows.length > 0);
  ...
  try {
    ...
    for (const passo of plano) {
      ...
      if (e) {
        ...
        await _marcarEnviado(tenantId, toqueId, { textoFinal, anexoId: anexo && anexo.id, erro: `texto não saiu: ${e}` });
    ...
    await _marcarEnviado(tenantId, toqueId, { textoFinal, anexoId: anexo && anexo.id, erro: `envio incompleto: ${err.message}` });
    return { ok: true, parcial: true, conversationId };
  }
  await _marcarEnviado(tenantId, toqueId, { textoFinal, anexoId: anexo && anexo.id, erro: null });
  return { ok: true, conversationId, enviados };
}

async function _marcarEnviado(tenantId, toqueId, { textoFinal, anexoId, erro }) {
  await withTenant(tenantId, (c) => c.query(
    `UPDATE lead_manager.boas_vindas_toque
        SET status = 'enviado', enviado_em = now(), bloqueio = NULL, texto_final = $3, anexo_id = $4, erro = $5, auto = false, updated_at = now()
      WHERE tenant_id = $1 AND id = $2`,
    [tenantId, toqueId, textoFinal, anexoId || null, erro]));
}
...
module.exports = { listarFila, detalhe, enviar, descartar, MOTIVO_BLOQUEIO, SQL_FILA };
