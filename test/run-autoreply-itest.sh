#!/usr/bin/env bash
# run-autoreply-itest.sh — itest da resposta automática fora do horário (a "Janis"), incluindo a trava
# dos assuntos que só a recepção responde (temaProibido.js). PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55463}"
CTR="lm-autoreply-itest-pg"
...
node --test test/autoreply.itest.js
