#!/usr/bin/env bash
# run-resources-ocupacao-recorrente-itest.sh — provisão da itest do endpoint per-recurso de
# ocupação recorrente. PG DESCARTÁVEL + migrations 046/048/050 (occupation_history + slot_end).
# NUNCA toca produção.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55435}"
CTR="lm-res-occ-itest-pg"
...
echo "[itest] bootstrap + migrations 046/048/050…"
...
SQL
for m in 046_resources_recorrente 048_resources_datado 050_occupation_slot_end; do
...
node --test test/resources-ocupacao-recorrente.itest.js
