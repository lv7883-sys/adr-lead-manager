...
TENANT_C="cccccccc-cccc-4ccc-8ccc-cccccccccccc"   # tenant FICTÍCIO novo (prova multi-tenant do seed)
...
SQL
for m in 051_contact_roles 060_cadastro_mestre 061_person_data_nascimento 067_contact_point_tipo 068_person_payer_relation 069_canonical_roles_seed; do
  docker exec -i "$CTR" psql -v ON_ERROR_STOP=1 -U postgres -d lm_itest < "$ROOT/db/migrations/${m}.sql" >/dev/null
done

# Papéis-base CANÔNICOS por-tenant via a FUNÇÃO (069) — sem hardcode por tenant. A/B recebem
# beneficiario/responsavel_financeiro/prestador. C (fictício) NÃO é semeado aqui: o itest chama
# seed_base_roles(C) em runtime pra provar o provisionamento multi-tenant.
docker exec -i "$CTR" psql -v ON_ERROR_STOP=1 -U postgres -d lm_itest >/dev/null <<SQL
