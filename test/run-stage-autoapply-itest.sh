#!/usr/bin/env bash
# run-stage-autoapply-itest.sh — itest da aplicação automática de sugestão de etapa (migration 078).
# PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55454}"
CTR="lm-stage-autoapply-itest-pg"
...
node --test test/stage-autoapply.itest.js
