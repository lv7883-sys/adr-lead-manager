#!/usr/bin/env bash
# run-reabordagem-cleanup-itest.sh — itest do #8 Fase 2 (dedup + coluna honesta + migração 073).
# PG DESCARTÁVEL, schema lead_manager (a 073 é schema-qualificada). NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55443}"
CTR="lm-reabordagem-itest-pg"
...
node --test test/reabordagem-cleanup.itest.js
