#!/usr/bin/env bash
# run-resources-salas-itest.sh — provisão da itest da tela SALA↔CAPABILITY (Unit 1).
# Sobe um Postgres DESCARTÁVEL (container efêmero), bootstrap + migration 046, exporta
# os envs e roda test/resources-salas.itest.js. NUNCA toca produção.
#
# Sem fixtures: a itest semeia as 9 salas + 22 capabilities direto no PG efêmero.
# Tenants em UUID v4 VÁLIDO (requireTenantAccess valida isUuid no tenantId da URL).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55433}"
CTR="lm-res-salas-itest-pg"
...
# pg_isready dá falso-positivo no server temporário do initdb (que o entrypoint
# REINICIA). Esperamos uma query REAL passar — e o server final ficar estável.
ready=0
...
node --test test/resources-salas.itest.js
