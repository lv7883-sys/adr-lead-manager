#!/usr/bin/env bash
# run-classificacao-bidirecional-itest.sh — itest da Fatia F (botão lead↔não-lead + Undo).
# PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55448}"
CTR="lm-classif-itest-pg"
...
node --test test/classificacao-bidirecional.itest.js
