#!/usr/bin/env bash
# run-inbox-thread-itest.sh — itest da thread do inbox (ADR-042 / E12-05: timeline compartilhada).
# PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55453}"
CTR="lm-inbox-thread-itest-pg"
...
node --test test/inbox-thread.itest.js
