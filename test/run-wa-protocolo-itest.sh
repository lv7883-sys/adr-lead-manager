#!/usr/bin/env bash
# run-waedicao-itest.sh — paridade 3b: edição e apagamento feitos no celular aplicados na original.
# PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55468}"
CTR="lm-waprotocolo-itest-pg"
...
node --test test/wa-protocolo.itest.js
