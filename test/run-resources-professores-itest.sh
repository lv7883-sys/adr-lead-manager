#!/usr/bin/env bash
# run-resources-professores-itest.sh — provisão da itest do endpoint /resources/professores.
# Sobe um Postgres DESCARTÁVEL (container efêmero), bootstrap + migration 046 (RLS FORCE),
# exporta os envs e roda test/resources-professores.itest.js. NUNCA toca produção.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55434}"
CTR="lm-res-profs-itest-pg"
...
node --test test/resources-professores.itest.js
