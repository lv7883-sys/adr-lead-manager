#!/usr/bin/env bash
# run-indicadores-fatiaE-itest.sh — itest da Fatia E (taxa envio-sem-erro + funil agendada).
# PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55447}"
CTR="lm-fatiae-itest-pg"
...
node --test test/indicadores-fatiaE.itest.js
