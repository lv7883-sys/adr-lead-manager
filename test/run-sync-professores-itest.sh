#!/usr/bin/env bash
# run-sync-professores-itest.sh — itest da GARANTIA do professor canônico (person + external_ref)
# + fechamento de service_account.professor_person_id. PG DESCARTÁVEL + migrations do cadastro
# (060…) + resources (046, p/ o mapa TEACHER) + 105 (colunas professor_*). NUNCA toca produção.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55440}"
CTR="lm-syncprof-itest-pg"
...
SQL
# cadastro (service_account/person/external_ref) + resources (mapa TEACHER) + colunas professor_*
for m in 051_contact_roles 060_cadastro_mestre 061_person_data_nascimento 067_contact_point_tipo 068_person_payer_relation 069_canonical_roles_seed 070_field_provenance 072_cadastro_sync 046_resources_recorrente 105_service_account_professor_person; do
...
node --test test/sync-professores.itest.js
