#!/usr/bin/env bash
# run-inbox-group-itest.sh — itest da ingestão de GRUPO (ADR-042 E14/B1 + paridade de mídia com o 1:1).
# PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55461}"
CTR="lm-inbox-group-itest-pg"
...
node --test test/inbox-group.itest.js
