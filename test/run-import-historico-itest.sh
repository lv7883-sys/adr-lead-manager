#!/usr/bin/env bash
# run-import-historico-itest.sh — itest do backfill de histórico (ADR-042): o importador avança
# conversations.last_read_at até o inbound mais novo da leva (histórico não conta como não-lido) e
# é monotônico (reimportar página antiga não recua o cursor). PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55452}"
CTR="lm-import-historico-itest-pg"
...
node --test test/import-historico.itest.js
