#!/usr/bin/env bash
# run-captura-duracao-itest.sh — provisão da itest da DURAÇÃO da ocupação (slot_end) na captura.
# Sobe um Postgres DESCARTÁVEL, bootstrap + migrations 046, 048 e 050 (slot_end), exporta os
# envs e roda test/captura-duracao.itest.js. HTML é MOCK (IO injetado) — NUNCA toca produção.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55436}"
CTR="lm-captura-duracao-itest-pg"
...
echo "[itest] bootstrap (role + db + lead_manager.tenants) + migrations 046, 048, 050…"
...
node --test test/captura-duracao.itest.js
