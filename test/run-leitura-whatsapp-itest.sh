#!/usr/bin/env bash
# run-waedicao-itest.sh — paridade 4: leitura e não lida vão do Regente para o WhatsApp.
# PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55469}"
CTR="lm-leitura-itest-pg"
...
node --test test/leitura-whatsapp.itest.js
