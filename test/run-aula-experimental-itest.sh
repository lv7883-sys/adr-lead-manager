#!/usr/bin/env bash
# run-aula-experimental-itest.sh — itest da PORTA DE SAÍDA da aula experimental (metrics.js).
# PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55472}"
CTR="lm-aula-experimental-itest-pg"
...
node --test test/aula-experimental.itest.js
