#!/usr/bin/env bash
# run-retomada-fonte-unica-itest.sh — itest do #8 Fatia B (re-fonte de retomada em metrics/plantão).
# PG DESCARTÁVEL, schema lead_manager. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55444}"
CTR="lm-retomada-itest-pg"
...
node --test test/retomada-fonte-unica.itest.js
