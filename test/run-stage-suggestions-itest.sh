#!/usr/bin/env bash
# run-stage-suggestions-itest.sh — itest do Passo 1 (fatia (b) — sugestões de etapa).
# PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55449}"
CTR="lm-stagesug-itest-pg"
...
node --test test/stage-suggestions.itest.js
