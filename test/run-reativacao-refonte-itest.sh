#!/usr/bin/env bash
# run-reativacao-refonte-itest.sh — itest da EMENDA #8 (Opção B): re-fonte da derivação do
# ciclo de reativação (staff_outbound_samples). PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55442}"
CTR="lm-reativacao-itest-pg"
...
node --test test/reativacao-refonte.itest.js
