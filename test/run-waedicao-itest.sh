#!/usr/bin/env bash
# run-waedicao-itest.sh — paridade 2: edição cifrada aplicada na mensagem original.
# PG DESCARTÁVEL. NUNCA toca produção.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PORT="${ITEST_PG_PORT:-55466}"
CTR="lm-waedicao-itest-pg"
...
node --test test/waEdicao.itest.js
