...
## Emenda 2026-07-20 — E1.3a fechado na ingestão de contrato (papel no /cadastro/contratos)
Contexto: o cadastro-mestre (ADR-037, fatia 037.2) já ingeria contratos em `/tenant/:tid/cadastro/contratos` gravando pessoa, contato (telefone alegado), contrato (`service_account`), o elo `account_member` e os `external_ref` — **mas NÃO escrevia `contact_role_member`**, a exata fonte que o Gate 0 lê (`_lookupRole`, engine.js). Era o gap do E1.3a: pessoa-aluno/responsável nascia sem papel, invisível ao gate. Confirmado read-only (2026-07-20).

Decisão (implementada): o endpoint passa a gravar o **papel** junto da ingestão.
•    `_upsertRole(c, tid, personId, telefone, roleKey)` novo, aditivo: ao criar/casar a pessoa-aluno grava `contact_role_member` papel `aluno`; ao casar o responsável, papel `responsavel`. Só ACRESCENTA — não toca `account_member`/`external_ref`/`service_account`.
•    Keyed por **telefone** (o modelo do 051: `UNIQUE (tenant_id, phone)`, match por dígitos BR-aware, como o `_lookupRole`). Sem telefone → não grava (menor sem fone próprio é reconhecido pelo telefone do responsável — regra E1.9). Idempotente por dígitos: telefone que já tem papel não é tocado (respeita a população 037.2a e "um papel por telefone"). `source='extranet_sync'`.
•    **Não inventa papel** (config-as-data, E1.5): exige a linha em `contact_role`. Prova: itest `cadastro-contratos` 4/4, incl. asserção de que `_lookupRole` resolve o aluno como `aluno`/`hard` após a ingestão.

Confirmação — **pagador NÃO depende de idade**: o vínculo é decidido pelo **dado**, não pela idade. O aluno é sempre `beneficiario`; `pagador` é criado só quando o item traz `responsavel` (i.e., a fonte tem `id_responsavel`/`tpresp≠0`). Nenhuma leitura de `dataNascimento` decide vínculo; `account_member` não tem coluna de idade. Isso realiza a **unidade de contato do E1.9** ("titular que consome + responsáveis que respondem pela conta") — inclusive o caso de **adulto com pagador distinto** (filho maior com contrato no nome do pai; empresa que paga o funcionário), que o modelo suporta e o endpoint trata sem ramo de idade. `tpresp` da Extranet (0=próprio / 1=outro aluno / 2=responsável) é o discriminante, não a data de nascimento. No sample do dry-run (30 fichas) `id_responsavel≠0` só apareceu em menores reais, mas o dado e o E1.9 admitem o caso adulto — o endpoint já está pronto para ele.

Dívida registrada — **seed de papéis-base por-tenant (multi-tenant)**: os papéis `aluno`/`responsavel` são semeados **só para Valinhos** (migração 060, `tenant_id` hardcoded); não há passo de onboarding que crie os papéis-base de um tenant novo. Consequência: para um 2º tenant, `_upsertRole` **não acha o papel e pula** — o endpoint **não quebra** (contratos/pessoas/vínculos gravam normalmente), só o papel fica ausente. Hoje esse skip é **logado** (`cadastro.role.nao_semeado`, `logger.warn`) para não ser invisível. O conserto correto **não** é `_upsertRole` criar papel na hora (violaria config-as-data e "papéis nunca do core") e sim um **seed de papéis-base por-tenant no onboarding** (o "template de vertical" do E1.5) — dívida a pagar **na chegada do 2º tenant da mesma vertical**, conforme o próprio E1.5 já previu. Enquanto Valinhos-only, o seed do 060 basta.

Reversão: só código (git revert + rebuild). Nenhuma migração; escrita aditiva e idempotente — desfazer o código deixa as linhas de papel já gravadas inertes (o gate segue em shadow/off).

## Emenda 2026-07-22 — `role_lead_policy` implementada (rescue questions = config-as-data por tenant)

A tabela `role_lead_policy` do E1.5 saiu do esboço para a **migração 071**, cravando a generalização multi-tenant que o E1.4/E1.5 já descreviam (a pergunta de resgate **nunca foi hardcode** — o classificador é role-cego; este é o primeiro lugar onde a pergunta por papel passa a existir, já como config por tenant).

**(a) Chave canônica, não rótulo de vertical.** `role_lead_policy.role` casa com a **key CANÔNICA** do `contact_role` — `prestador` / `beneficiario` / `responsavel_financeiro` — não os rótulos de vertical (`professor`/`aluno`) do esboço. O vocabulário de escola vive no `label` do papel, nunca na chave. Mantém "professor/aluno são conceito de vertical, não do core".

**(b) Template de vertical = seed, não tabela (conforme E1.5).** `seed_role_lead_policy_escolas(tenant)` semeia os 3 papéis da vertical "escolas" com `default_action` + `rescue_prompt` + `rescue_scope`; idempotente; o tenant pode **sobrescrever** a linha. A "tabela real de vertical da qual o tenant herda" segue como **dívida do 2º tenant** (E1.5 inalterado). Outra vertical terá sua própria função de seed.

**(c) Default seguro quando um tenant não tem política pra um papel.** O motor (`src/roleLeadPolicy.js`, `getRescuePolicy(tenant, role)`) lê a linha do **tenant atual** sob RLS; **sem linha → `{ default_action: 'vigia', rescue_prompt: null }`** — `vigia` = **nunca descarte cego** (vai pro Revisar / humano decide), sem `rescue_prompt` o classificador segue genérico. Escolha conservadora (precisão sobre recall p/ conhecido, coerente com o rollout). O skip é **logado** (`rescue.policy.nao_semeada`) para tornar visível a falta do seed por-tenant.

**(d) Escopo desta fatia.** Migração 071 (tabela + seed + seed retroativo do Valinhos) + motor de LEITURA + itest (multi-tenant real: tenant fictício de outra vertical usa perguntas próprias; cross-tenant=0; default seguro). O motor **ainda não injeta** a pergunta no classificador nem age no gate — o gate segue em **shadow**. **Zero mudança de comportamento**; é a fundação config-as-data. Reversão: `DROP` da 071 + git revert (aditivo).
